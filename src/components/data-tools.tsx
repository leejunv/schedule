...
export function DataTools() {
  ...
  function resetData() {
    if (!window.confirm("현재 계정의 일정 데이터를 초기화할까요?")) return;
    state.reset();
  }
  ...
  return (
    <section className="rounded-lg border border-black/10 bg-white p-4 shadow-soft dark:border-white/10 dark:bg-[#191d23]">
      <div className="mb-4">
        ...
        <p className="text-sm text-[#68707c] dark:text-[#aeb6bd]">Firebase 계정 데이터 백업 및 복원</p>
      </div>
      <div className="grid gap-2">
        ...
        <button type="button" onClick={resetData} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-[#d76b4f]/30 px-3 font-medium text-[#b94e33] hover:bg-[#d76b4f]/10">
          <RotateCcw size={17} />
          계정 데이터 초기화
        </button>
      </div>
      <div className="mt-4 rounded-lg bg-[#f7f5f0] p-3 text-sm text-[#68707c] dark:bg-[#111418] dark:text-[#aeb6bd]">
        Google 로그인 후에는 같은 계정의 PC와 모바일에서 일정이 Firestore로 자동 동기화됩니다. JSON 내보내기/가져오기는 백업용으로 사용할 수 있습니다.
      </div>
    </section>
  );
}
